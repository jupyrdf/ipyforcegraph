...
*** Test Cases ***
ForceGraph Can Set Link color to str('blue')
    Shape Link Feature Works As Expected    ForceGraph    color    str    blue    1

ForceGraph Can Set Link color to B.Column('link_color')
    Shape Link Feature Works As Expected    ForceGraph    color    B.Column    link_color    2

ForceGraph Can Set Link color to B.Nunjucks('{{ link.link_color }}')
    Shape Link Feature Works As Expected    ForceGraph    color    B.Nunjucks    {{ link.link_color }}    3

ForceGraph Can Set Link curvature to int('1')
    Shape Link Feature Works As Expected    ForceGraph    curvature    int    1    1

ForceGraph Can Set Link curvature to float('2.5')
    Shape Link Feature Works As Expected    ForceGraph    curvature    float    2.5    2

ForceGraph Can Set Link curvature to B.Column('value')
    Shape Link Feature Works As Expected    ForceGraph    curvature    B.Column    value    3

ForceGraph Can Set Link curvature to B.Nunjucks('{{ link.value * 1.5 }}')
    Shape Link Feature Works As Expected    ForceGraph    curvature    B.Nunjucks    {{ link.value * 1.5 }}    4

ForceGraph Can Set Link line_dash to B.Nunjucks('{{ [10, 2, 10] }}')
    Shape Link Feature Works As Expected    ForceGraph    line_dash    B.Nunjucks    {{ [10, 2, 10] }}    1

ForceGraph Can Set Link width to int('2')
    Shape Link Feature Works As Expected    ForceGraph    width    int    2    1

ForceGraph Can Set Link width to float('3.5')
    Shape Link Feature Works As Expected    ForceGraph    width    float    3.5    2

ForceGraph Can Set Link width to B.Column('value')
    Shape Link Feature Works As Expected    ForceGraph    width    B.Column    value    3

ForceGraph Can Set Link width to B.Nunjucks('{{ link.value * 0.5 }}')
    Shape Link Feature Works As Expected    ForceGraph    width    B.Nunjucks    {{ link.value * 0.5 }}    4
